package gear.utils {
	...
	public final class GBDUtil {
		...
		//XIEJ PLUS 2012-12-13
		//调整位置(无效的方法)
		public static function getSpaceBD(source : IBitmapDrawable, x:int = 0, y:int = 0, location:String = "") : BitmapData {
			if (source == null) {
				return null;
			}
			var sx : int = 0;
			...
			if (source is DisplayObject) {
				sx = DisplayObject(source).x;
				sy = DisplayObject(source).y;
			...
			mtx.transformPoint(new Point(sx, sy));
			
			var target : BitmapData = new BitmapData(sw, sh, true, 0);
